...
class InventoryScreen:
    def __init__(self, goto_mainmenu):
        ...
        # Title
        title = Label(rect=(640, 30, 100, 100), text="Targyak", font=BP, color=(255, 255, 255))
        ...
        # Assign static defense values to each armor
        import random
        random.seed(42)  # Fixed seed for consistent values
        self.armor_defense = {}
        for i, armor in enumerate(PLAYERARMOR):
            self.armor_defense[i] = random.randint(10, 25)
    ...
    def AddItem(self, idx):
        ...
        defense = self.armor_defense[idx]
    ...
    def handle_event(self, e):
        ...
        # Handle clicks on equipped items to unequip
        if e.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.inventory_area.collidepoint(mouse_pos):
                for slot in self.slots:
                    ...
                    slot_y = self.inventory_area.y + slot['y_offset'] - self.scroll_y + 10
                    slot_rect = pygame.Rect(slot_x, slot_y, slot['width'], slot['height'])
    ...
    def draw(self, surf):
        ...
        self.CreateItemSlot(surf, color=(255, 255, 255), start=(200, 150), end=(500, 700), thickness=3, fill_color=(20, 20, 30))
        ...
        if len(EquipedItems) > 0:
            for item in EquipedItems:
                ...
                if item_what == "mellvert":
                    ...
                    pos = (240, 351)
                    surf.blit(img, pos)
                    self.equipped_item_rects["mellvert"] = pygame.Rect(pos[0], pos[1], 225, 150)
                elif item_what == "sapka":
                    img1 = pygame.transform.scale(item_img, (175, 130))
                    pos = (270, 200)
                    surf.blit(img1, pos)
                    self.equipped_item_rects["sapka"] = pygame.Rect(pos[0], pos[1], 175, 130)
                elif item_what == "cipo":
                    # Draw boots BEFORE leggings so leggings appear on top
                    img3 = pygame.transform.scale(item_img, (175, 100))
                    pos = (270, 500)
                    surf.blit(img3, pos)
                    self.equipped_item_rects["cipo"] = pygame.Rect(pos[0], pos[1], 175, 100)
                elif item_what == "nadrag":
                    # Draw leggings AFTER boots so they appear on top
                    img2 = pygame.transform.scale(item_img, (175, 100))
                    pos = (270, 450)
                    surf.blit(img2, pos)
                    self.equipped_item_rects["nadrag"] = pygame.Rect(pos[0], pos[1], 175, 100)
        ...
        if not self.slots:
            ...
            empty_text = BP_SMALL.render("Nincs targy - menj a boltba!", True, (200, 200, 200))
            text_rect = empty_text.get_rect(center=(self.inventory_area.centerx, self.inventory_area.centery))
            ...
            for slot in self.slots:
                ...
                if slot_y + slot['height'] < self.inventory_area.y or slot_y > self.inventory_area.bottom:
                    continue
                ...
                defense = self.armor_defense[slot['index']]
                defense_text = BP_SMALL.render(f"{defense}%", True, (255, 255, 0))
        ...
        if self.max_scroll > 0:
            ...
            # Background
            pygame.draw.rect(surf, (50, 50, 50), (scrollbar_x, scrollbar_y, 10, scrollbar_height))
            
            # Handle
            handle_height = max(30, int((self.inventory_area.height / (self.inventory_area.height + self.max_scroll)) * scrollbar_height))
            handle_y = scrollbar_y + int((self.scroll_y / self.max_scroll) * (scrollbar_height - handle_height))
            pygame.draw.rect(surf, (255, 50, 50), (scrollbar_x, handle_y, 10, handle_height))
    ...
    def refresh_inventory(self):
        ...
        # Reassign defense values for new items
        import random
        random.seed(42)
        self.armor_defense = {}
        for i, armor in enumerate(PLAYERARMOR):
